...
```{r message = FALSE}
# Retrieve and process supplementary files from GEO
...
if (!file.exists("raw_counts.RData")) {
  ...
  untar(file.path(dataset_geoID, data_filename), exdir = "GSE273848_extracted")
  
  raw_count_data <- read.delim(file.path("GSE273848_extracted", data_filename), header=TRUE)
...
boxplot(log2(cpm(raw_count_data)), main="Raw Count Distribution", col="lightblue")
```
...
id_conversion <- getBM(attributes = c("ensembl_gene_id", "hgnc_symbol"), filters = "ensembl_gene_id", values = rownames(normalized_counts), mart = ensembl_human)
